...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/origin-select"
...
export default function DateTimePicker({ date, setDate }: DateTimePickerProps) {
    ...
    React.useEffect(() => {
        if (date) {
            ...
            setTime((prev) => {
                if (prev.h === newH && prev.m === newM && prev.ampm === ampm) return prev;
        ...
        <div className="flex flex-col gap-4">
            ...
            <Popover>
                <PopoverTrigger asChild>
                    <Button
                        variant="outline"
                        className={cn("w-full justify-start text-left font-normal border-white/10 bg-[#2c2c2e] text-gray-200 hover:bg-[#3c3c3e] hover:text-gray-100", !date && "text-muted-foreground")}
                    >
            ...
            <div className="flex items-center gap-2">
                ...
                <Select value={time.h} onValueChange={(v) => handleTimeChange('h', v)}>
                    ...
                    <SelectContent className="bg-[#1c1c1e] border-white/10 text-gray-200">
                        {Array.from({ length: 12 }, (_, i) => {
                            const h = i + 1
                            return (
                                <SelectItem key={h} value={h.toString().padStart(2, "0")} className="focus:bg-[#2c2c2e] focus:text-white">
                ...
                <Select value={time.m} onValueChange={(v) => handleTimeChange('m', v)}>
                    ...
                    <SelectContent className="bg-[#1c1c1e] border-white/10 text-gray-200">
                        {["00", "15", "30", "45"].map((m) => (
                            <SelectItem key={m} value={m} className="focus:bg-[#2c2c2e] focus:text-white">
                                {m}
                            </SelectItem>
                        ))}
                    </SelectContent>
                ...
                <Select value={time.ampm} onValueChange={(v) => handleTimeChange('ampm', v)}>
                    <SelectTrigger className="w-[70px] border-white/10 bg-[#2c2c2e] text-gray-200">
                        <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="bg-[#1c1c1e] border-white/10 text-gray-200">
                        <SelectItem value="AM" className="focus:bg-[#2c2c2e] focus:text-white">AM</SelectItem>
                        <SelectItem value="PM" className="focus:bg-[#2c2c2e] focus:text-white">PM</SelectItem>
                    </SelectContent>
                </Select>
            </div>
        </div>
