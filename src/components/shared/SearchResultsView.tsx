...
import { getBaseColumns, PartStatusRenderer } from "@/components/shared/GridConfig";
...
const SourceRenderer = (params: any) => {
    ...
    if (source === "Main Sheet") colorClass = "bg-indigo-500/10 text-indigo-400 border-indigo-500/20";
    if (source === "Orders") colorClass = "bg-orange-500/10 text-orange-400 border-orange-500/20";
    if (source === "Booking") colorClass = "bg-purple-500/10 text-purple-400 border-purple-500/20";
    if (source === "Call") colorClass = "bg-blue-500/10 text-blue-400 border-blue-500/20";
    if (source === "Archive") colorClass = "bg-slate-500/10 text-slate-400 border-slate-500/20";
        ...
        <div className="flex flex-col h-full bg-[#0a0a0b] text-white">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#0a0a0b]/50 backdrop-blur-xl">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-lg font-semibold text-white/90">Global Search Results</h2>
                        <div className="flex items-center gap-3 mt-1 text-sm text-gray-400">
                            <span className="text-white font-medium">{searchResults.length}</span> matches for
                            <span className="px-1.5 py-0.5 rounded bg-white/10 text-white font-mono text-xs">"{searchTerm}"</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    {Object.entries(counts).map(([source, count]) => (
                        <div key={source} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/5 text-xs text-gray-300">
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1 max-w-xs">
                                We couldn't find any records matching "{searchTerm}". Try different keywords or check spelling.
                            </p>
                        </div>
                        <Button
                            onClick={handleClearSearch}
