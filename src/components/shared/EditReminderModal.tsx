...
import { Bell, Trash2, Plus, Clock, Calendar as CalendarIcon } from "lucide-react";
...
interface EditReminderModalProps {
    ...
    onSave: (data: { date: string; time: string; subject: string } | undefined) => void;
}
...
export const EditReminderModal = ({
    ...
    const { reminderTemplates, addReminderTemplate, removeReminderTemplate } = useAppStore();
                    ...
                    {showDeleteConfirm && (
                        <div className="absolute inset-0 z-50 flex items-center justify-center p-6 bg-[#1c1c1e]/80 backdrop-blur-sm animate-in fade-in duration-200">
                            <div className="bg-[#2c2c2e] border border-white/10 rounded-xl p-6 shadow-2xl w-full max-w-[280px] text-center space-y-4 animate-in zoom-in-95 duration-200 border-red-500/20">
                                ...
                                <div>
                                    <h3 className="text-sm font-semibold text-white">Clear Reminder?</h3>
                                    <p className="text-xs text-gray-400 mt-1">This will permanently remove the reminder from this row.</p>
                                </div>
