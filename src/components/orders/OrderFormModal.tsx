...
export const OrderFormModal = ({
	...
	// Async database-backed duplicate warnings
	 ...
	 * This augments the in-memory check by querying Supabase directly.
	 */
	...
	// Debounced VIN check for all parts
	useEffect(() => {
		const timer = setTimeout(() => {
			if (formData.vin.length >= 6) {
				for (const part of parts) {
					if (part.partNumber.trim()) {
						checkDuplicateForPart(part.id, formData.vin, part.partNumber);
					}
				}
			}
		}, 800);
