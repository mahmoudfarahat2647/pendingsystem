...
import { getBaseColumns, getOrdersColumns, getMainSheetColumns, getBookingColumns } from "../components/shared/GridConfig";

// Mock the store
vi.mock("@/store/useStore", () => ({
    useAppStore: Object.assign(
        (fn: any) => fn({ bookingStatuses: [] }),
...
describe("Grid Column Configuration Selection Fix", () => {
    describe("getBaseColumns", () => {
        it("should have headerCheckboxSelectionFilteredOnly set to true in the selection column", () => {
            ...
            // @ts-ignore - headerCheckboxSelectionFilteredOnly might not be in the ColDef types if using an older version but it is a valid ag-grid property
    ...
    describe("useColumnDefs hook", () => {
        it("should include headerCheckboxSelectionFilteredOnly: true for all grid types", () => {
            const gridTypes: Array<"main" | "orders" | "booking" | "archive" | "call"> = [
                "main", "orders", "booking", "archive", "call"
            ];

            for (const type of gridTypes) {
                const { result } = renderHook(() => useColumnDefs(type));
                const selectionCol = result.current.find(col => col.colId === "selection");

                expect(selectionCol, `Selection column missing for type ${type}`).toBeDefined();
                expect(selectionCol?.headerCheckboxSelection).toBe(true);
                // @ts-ignore
                expect(selectionCol?.headerCheckboxSelectionFilteredOnly).toBe(true);
            }
