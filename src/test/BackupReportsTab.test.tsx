...
describe("BackupReportsTab", () => {
	it("should render all child components when not locked", () => {
		...
		expect(screen.getByTestId("scheduling-card")).toHaveAttribute(
			"data-locked",
			"false",
		);
		expect(screen.getByTestId("recipients-card")).toHaveAttribute(
			"data-locked",
			"false",
		);
		expect(screen.getByTestId("manual-action-card")).toHaveAttribute(
			"data-locked",
			"false",
		);
	});
	...
	it("should render scheduling and recipients cards in the same grid row", () => {
		...
		expect(gridContainer).toContainElement(
			screen.getByTestId("scheduling-card"),
		);
		expect(gridContainer).toContainElement(
			screen.getByTestId("recipients-card"),
		);
	});
	...
	it("should render manual action card outside the grid", () => {
		...
		expect(gridContainer).not.toContainElement(
			screen.getByTestId("manual-action-card"),
		);
