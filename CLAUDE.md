...
- **Shared UI**: `MoveToMainButton` (localized tooltip + `aria-label`) on all four toolbars, and `MoveToMainConfirmDialog` — a plain Yes/No `ConfirmDialog` (no field validation, no type-to-confirm) that re-checks the switch on Yes and aborts with a toast if it was turned off. Not restricted to a single VIN (Booking's mixed-VIN guard does not apply). Button label, tooltip, confirmation, and toast feedback are translated EN/AR under `modals.moveToMain.*` / `modals.stageConfirm.moveToMain*` — an explicit exception to the usual "toolbars and toasts stay English" rule, per issue `#314`.
