...
### Localization (English/Arabic) — Foundation (`#263`, wave 1 of parent `#255`)

First-class English/Arabic localization is being rolled out under parent issue `#255`. **`#263` (this wave, delivered)** built the foundation only — the provider, persisted preference, catalogs, and the Settings language control — and translated just enough surface (Settings modal, sidebar nav labels, one Header string) to prove the mechanism end-to-end. Full product-wide translation coverage (grids, forms, notifications, RTL component behavior, formatting) is later-wave tickets under `#255` and is **not** done yet.
