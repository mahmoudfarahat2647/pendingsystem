...
- **Not yet delivered (tracked as GitHub issues, open as of 2026-09-15):** `#204` end-to-end verification + rollout-order check. Full spec: issue `#191`.
...
- FREEZE stage transitions are now guarded by compare-and-set (single-row and bulk, including the batched rollback path) and realtime sync reacts to `UPDATE`/`DELETE` as well as `INSERT` — delivered by `#202` (2026-09-14). `#204` end-to-end verification + rollout-order check remains open. See [FREEZE Workflow](#freeze-workflow).
