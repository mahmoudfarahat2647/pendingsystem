...
### Localization (English/Arabic) — Foundation (`#263`) + RTL component behavior (`#265`), waves of parent `#255`

First-class English/Arabic localization is being rolled out under parent issue `#255`. **`#263`** built the foundation — the provider, persisted preference, catalogs, and the Settings language control — and translated just enough surface (Settings modal, sidebar nav labels, one Header string) to prove the mechanism end-to-end. **`#265` (delivered)** made the interface *behave* RTL in Arabic, not just look it. Full product-wide translation coverage (grids, forms, notifications, formatting) is still a later-wave ticket under `#255` and is **not** done yet.

- **Domain**: `src/domain/locale/locale.ts` — `Locale` (`"en" | "ar"`), `DEFAULT_LOCALE` (`"en"`), `isLocale`/`normalizeLocale` (malformed/legacy/missing → English), `getDirection` (`"ar"` → `"rtl"`, else `"ltr"`). Pure, no React/storage.
- **Catalogs**: `src/locales/{en,ar}.ts` implement `TranslationCatalog` (`src/locales/types.ts`) as a typed object literal — a missing or extra key fails `type-check` via TypeScript excess-property checking. `src/locales/index.ts`'s `translate(locale, key)` does the dotted-path lookup with fallback to English, then to the raw key, and never throws or renders blank.
- **Provider**: `src/components/providers/LocaleProvider.tsx`, mounted once in `src/app/layout.tsx` (covers protected app routes, public auth/reset routes, and mobile intake alike — one root layout). Exposes `{ locale, dir, setLocale, t }` via `src/hooks/useTranslation()` (context object lives in `src/hooks/localeContext.ts` so hooks never import from `components/`). Startup boundary: both the server render and the first client render use `DEFAULT_LOCALE`; the persisted preference is applied inside a `useLayoutEffect` (before paint), which is also where `document.documentElement.lang`/`dir` are set imperatively — never as JSX props on `<html>`, so there is no hydration-mismatch risk. Switching locale updates the same mounted context/tree; nothing is remounted. Since `#265`, the same provider also wraps `children` in Radix's `DirectionProvider` (`@radix-ui/react-direction`) and Base UI's `DirectionProvider` (`@base-ui/react/direction-provider`) — every installed Dialog/DropdownMenu/Select/Popover reads direction through its own supported context, not `document.dir` alone.
- **Persistence**: `locale` lives in the existing Zustand store (`src/store/useStore.ts`, key `pending-sys-storage-v1.1`) as a normal field in `uiSlice.ts`, included in the existing `partialize` allowlist. A custom `merge` option normalizes `state.locale` on **every** hydration (not just on a version-migration path), so a malformed/legacy locale inside an otherwise-valid current-version snapshot self-heals to English while every other persisted field (including the v0→v1 part-status migration) round-trips untouched. `storage` wraps `localStorage` in try/catch (`safeStorage`) so a quota-limited/unavailable store never throws — switching language always succeeds in memory even when the disk write is skipped. Sign-out (`SidebarUserMenu`) never touches this store, so the saved locale survives it.
- **Settings control**: `src/components/shared/settings/LanguageTab.tsx` — a `role="radiogroup"` of two real `<input type="radio">` options (native English / native Arabic names), with an explicit arrow-key handler layered on top of native radio semantics. It takes no `isLocked` prop: language is a presentation preference, available even while Settings is locked.
- **No locale-prefixed routes**: URLs, query keys, and stage keys are unchanged; only presentation (document `lang`/`dir` and translated copy) changes.
- **Bidi isolation (`#265`)**: `src/components/shared/bidi/LtrIsolate.tsx` forces `dir="ltr"` + `unicode-bidi: isolate` (no bidi control characters, so copied text is byte-for-byte identical in both locales) around VINs, part numbers, phone numbers, tracking IDs and the release confirmation token — applied in the release confirmation modal, the duplicate-order warning dialog, notifications dropdown, and booking sidebar details, plus `dir="ltr"` on the corresponding order-form inputs. Operator free-text fields (description, customer name) use `dir="auto"` instead.
- **Directional icons (`#265`)**: icons whose meaning is directional (sidebar collapse chevron, calendar/booking month navigation, dropdown submenu arrow, Freeze "move to" arrow) carry a `rtl:-scale-x-100` class that mirrors them under an RTL ancestor; neutral icons (Check, Snowflake, ChevronUp/Down, etc.) are left unchanged.
- **Logical CSS (`#265`)**: the dialog, alert-dialog, dropdown-menu, calendar and command primitives use logical Tailwind utilities (`start`/`end`, `ps`/`pe`, `ms`/`me`) instead of `left`/`right`/`pl`/`pr`/`ml`/`mr` for anything direction-dependent — arbitrary flex/grid structures are not reversed.
- **AG Grid stays LTR (`#265` addendum, 2026-09-22)**: the grid boundary is intentionally excluded from RTL — no `enableRtl`, no mirrored columns; existing column positions/order/widths/pinning/alignment/behavior are unchanged. This also means VIN/part-number/etc. **cells** and **search results** (rendered through `SearchResultsGrid`, also AG Grid) are already LTR by construction and needed no additional bidi wrapping. Product-owned column header translation is a separate, later ticket (`#271`).
- Tests: `src/test/locale/` — catalog parity/type-safety (`catalogParity.test.ts`), persistence across a simulated remount incl. malformed/legacy/corrupt-JSON/quota-exceeded storage (`localePersistence.test.ts`), the `LanguageTab` control in isolation (`LanguageTab.test.tsx`), and a shell-level integration test through the real provider + real store (`localeShellIntegration.test.tsx`). `src/test/rtl/` (`#265`) — direction reaching both primitive libraries' own context (`directionProviders.test.tsx`), a real portaled Dialog/DropdownMenu+Popover/Select exercised in Arabic incl. arrow-key nav and a submenu ArrowLeft-opens test (`portaledPrimitives.test.tsx`), one directional icon vs. one neutral icon (`directionalIcons.test.tsx`), and `LtrIsolate`'s no-bidi-control-character guarantee (`LtrIsolate.test.tsx`). `src/test/setup.ts` gained jsdom pointer-capture/`scrollIntoView` shims, required for any test that opens a Radix interactive primitive.
