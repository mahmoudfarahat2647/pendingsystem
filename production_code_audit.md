...
### ~~High: Public mobile-order route depends on service-role writes plus process-local rate limiting~~ ✅ RESOLVED
...
- **Resolution (2026-05-16):** Two-part fix applied. (1) **Cross-instance rate limiting:** Replaced the process-local `Map` with a Supabase-backed rolling-window implementation — a `rate_limits` table stores one row per request with a `requested_at TIMESTAMPTZ` column; the `check_rate_limit(p_ip, p_max_requests)` RPC atomically prunes stale rows, inserts, and counts within the trailing hour in a single round-trip, enforcing a true sliding 30-req/hour window across all serverless instances (migration `20260516_rate_limits_fix.sql`). (2) **Permissions lockdown:** The `rate_limits` table and both RPCs (`check_rate_limit`, `prune_rate_limits`) have all access revoked from `PUBLIC`, `anon`, and `authenticated`; only `service_role` (used by the server-side `createServiceClient()`) retains access. The RPC uses `SECURITY DEFINER` as defense in depth.

### ~~High: Settings/report data is mutable through client-side-only gates~~ ✅ RESOLVED
...
- **Resolution (2026-05-15):** The client-side password was always intentional as a UI convenience only — the real authorization gate is the Better Auth session check enforced server-side in `/api/report-settings` and `/api/app-settings` (both return 401 without a valid session). All direct anon write access to `report_settings` was removed by migration `20260515_secure_settings_rls.sql` (covered under the Critical finding above), closing the "anon RLS" gap cited in the impact. Data never reaches the database without passing through a session-validated, service-role-keyed API route.
