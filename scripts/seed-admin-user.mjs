...
if (!adminEmail || !adminPassword) {
  console.error("AUTH_ADMIN_EMAIL and AUTH_ADMIN_PASSWORD must be set in .env.local");
  process.exit(1);
}
...
if (existing.rows.length > 0) {
  ...
  console.log(`✓ Admin user already exists: ${user.email} (username: ${user.username})`);
  await pool.end();
  process.exit(0);
}

// Create the admin user
console.log(`Creating admin user: ${adminEmail} (username: ${adminUsername})...`);
